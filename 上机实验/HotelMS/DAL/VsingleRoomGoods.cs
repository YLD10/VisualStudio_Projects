...
namespace Maticsoft.DAL
{
	...
	public partial class VsingleRoomGoods
	{
		...
		public bool Exists(string goodsID,string roomTypeID)
		{
			...
			strSql.Append("select count(1) from VsingleRoomGoods");
		...
		public bool Add(Maticsoft.Model.VsingleRoomGoods model)
		{
			...
			strSql.Append("insert into VsingleRoomGoods(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update VsingleRoomGoods set ");
			if (model.goodsName != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from VsingleRoomGoods ");
			strSql.Append(" where goodsID='"+goodsID+"' and roomTypeID='"+roomTypeID+"' " );
			int rowsAffected=DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
		...
		public Maticsoft.Model.VsingleRoomGoods GetModel(string goodsID,string roomTypeID)
		{
			...
			strSql.Append(" from VsingleRoomGoods ");
			strSql.Append(" where goodsID='"+goodsID+"' and roomTypeID='"+roomTypeID+"' " );
			Maticsoft.Model.VsingleRoomGoods model=new Maticsoft.Model.VsingleRoomGoods();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.VsingleRoomGoods DataRowToModel(DataRow row)
		{
			Maticsoft.Model.VsingleRoomGoods model=new Maticsoft.Model.VsingleRoomGoods();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from VsingleRoomGoods T ");
