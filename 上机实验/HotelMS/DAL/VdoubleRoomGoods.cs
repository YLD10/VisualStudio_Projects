...
namespace Maticsoft.DAL
{
	...
	public partial class VdoubleRoomGoods
	{
		...
		public bool Exists(string goodsID,string roomTypeID)
		{
			...
			strSql.Append("select count(1) from VdoubleRoomGoods");
		...
		public bool Add(Maticsoft.Model.VdoubleRoomGoods model)
		{
			...
			strSql.Append("insert into VdoubleRoomGoods(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update VdoubleRoomGoods set ");
			if (model.goodsName != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from VdoubleRoomGoods ");
			strSql.Append(" where goodsID='"+goodsID+"' and roomTypeID='"+roomTypeID+"' " );
			int rowsAffected=DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
		...
		public Maticsoft.Model.VdoubleRoomGoods GetModel(string goodsID,string roomTypeID)
		{
			...
			strSql.Append(" from VdoubleRoomGoods ");
			strSql.Append(" where goodsID='"+goodsID+"' and roomTypeID='"+roomTypeID+"' " );
			Maticsoft.Model.VdoubleRoomGoods model=new Maticsoft.Model.VdoubleRoomGoods();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.VdoubleRoomGoods DataRowToModel(DataRow row)
		{
			Maticsoft.Model.VdoubleRoomGoods model=new Maticsoft.Model.VdoubleRoomGoods();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from VdoubleRoomGoods T ");
