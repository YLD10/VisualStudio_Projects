...
namespace Maticsoft.DAL
{
	...
	public partial class Vunavailiable
	{
		...
		public bool Exists(string floorNum,string roomID)
		{
			...
			strSql.Append("select count(1) from Vunavailiable");
		...
		public bool Add(Maticsoft.Model.Vunavailiable model)
		{
			...
			strSql.Append("insert into Vunavailiable(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Vunavailiable set ");
			if (model.liveState == 0 || model.liveState == 1)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Vunavailiable ");
			strSql.Append(" where floorNum='"+floorNum+"' and roomID='"+roomID+"' " );
			int rowsAffected=DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
		...
		public Maticsoft.Model.Vunavailiable GetModel(string floorNum,string roomID)
		{
			...
			strSql.Append(" from Vunavailiable ");
			strSql.Append(" where floorNum='"+floorNum+"' and roomID='"+roomID+"' " );
			Maticsoft.Model.Vunavailiable model=new Maticsoft.Model.Vunavailiable();
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.Vunavailiable DataRowToModel(DataRow row)
		{
			Maticsoft.Model.Vunavailiable model=new Maticsoft.Model.Vunavailiable();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Vunavailiable T ");
