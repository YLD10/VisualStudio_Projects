...
namespace Maticsoft.DAL
{
	...
	public partial class CustomerLive
	{
		...
		public bool Exists(DateTime startLiveTime, string cusID, string floorNum, string roomID)
		{
			...
			strSql.Append("select count(1) from CustomerLive");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("delete from CustomerLive ");
			strSql.Append(" where startLiveTime='" + startLiveTime + "' and cusID='" + cusID + "' and floorNum='" + floorNum + "' and roomID='" + roomID + "' ");
			int rowsAffected = DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rowsAffected > 0)
			{
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select startLiveTime,proEndLiveTime,endLiveTime,cusID,floorNum,roomID,isAccount ");
			strSql.Append(" FROM CustomerLive ");
			if (strWhere.Trim() != "")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from CustomerLive T ");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select cusID 身份证号,floorNum 楼层号,roomID 房间号,startLiveTime 入住时间,proEndLiveTime 预定退房时间,endLiveTime 实际退房时间,(case isAccount when 1 then '是' else '否' end )是否已结算  ");
			strSql.Append(" FROM CustomerLive ");
			if (strWhere.Trim() != "")
			{
				strSql.Append(" where " + strWhere);
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "CustomerLive";
